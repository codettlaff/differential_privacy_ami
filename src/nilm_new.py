...
def precompute_indices_new(num_timesteps, window_length, stride, train_val_test_split, number_blocks, seed=42):
    # More blocks improves distribution (less seasonal drift) but means we lose more timesteps to gaurding
    # Not all blocks need to be the same number of timesteps (need to make sure we satisfy train val test split)
    # Can replace number_blocks with a different setting if that makes more sense.
    
    center_offset = window_length // 2 
    gaurd = window_length - 1
    rng = np.random.default_rng(seed)
    
    # Number of blocks in each split
    n_train_rounds = int(round(train_val_test_split[0] * number_blocks))
    ...
    n_val = int(train_val_test_split[1] * number_blocks))
    n_test_blocks = number_blocks - n_train_blocks - n_val_blocks
